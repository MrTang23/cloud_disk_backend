...
def download_small_file_content(request):
    ...
    if not user_id or not file_id:
        return global_function.json_response('', '缺少用户 ID 或文件 ID', status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        return global_function.json_response('', '无效的用户 ID 或文件 ID', status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        return global_function.json_response('', '文件不存在或不属于该用户', status.HTTP_404_NOT_FOUND)

    # 检查文件是否存在于文件系统
    file_path = file_instance.path
    if not os.path.exists(file_path):
        return global_function.json_response('', '文件系统中未找到文件', status.HTTP_404_NOT_FOUND)
    ...
    try:
        ...
        return global_function.json_response('', '文件读取出错', status.HTTP_500_INTERNAL_SERVER_ERROR)
    except Exception as e:
        return global_function.json_response('', f'文件下载出错: {str(e)}', status.HTTP_500_INTERNAL_SERVER_ERROR)
